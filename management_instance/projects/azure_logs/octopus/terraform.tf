...
resource "octopusdeploy_project" "project_provision" {
  name                                 = "Provision Azure Logs"
...
resource "octopusdeploy_variable" "amazon_web_services_account_variable" {
  owner_id  = octopusdeploy_project.project_provision.id
  type      = "AmazonWebServicesAccount"
  name      = "AWS"
  value     = data.octopusdeploy_accounts.aws.accounts[0].id
}

resource "octopusdeploy_deployment_process" "deployment_process_project_provision" {
  project_id = "${octopusdeploy_project.project_provision.id}"
  ...
  step {
    ...
    action {
      ...
      properties                         = {
        ...
        "Octopus.Action.Terraform.TemplateDirectory" = "managed_instances/projects/azure_logs/s3backend"
        ...
        "Octopus.Action.Terraform.AdditionalInitParams" = "-backend-config=\"key=managed_instance_project_azure_logs\" -backend-config=\"bucket=${var.bucket_name}\" -backend-config=\"region=${var.bucket_region}\""
