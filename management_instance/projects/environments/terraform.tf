  ...
  step {
    ...
    action {
      ...
      properties                         = {
        ...
        "Octopus.Action.Terraform.AdditionalInitParams" = "-backend-config=\"key=managed_instance_project_environments\" -backend-config=\"bucket=$${{ secrets.BUCKET_NAME }}\" -backend-config=\"region=$${{ secrets.BUCKET_REGION }}\""
